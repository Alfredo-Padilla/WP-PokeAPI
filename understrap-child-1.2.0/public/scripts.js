...
function addListenersToFilters() {
  ...
  let type = urlParams.get('types') || 'all';
  console.log(type);
  // Add listeners
...
function addAttackCallback() {
  ...
  newAttack.querySelector('.remove-attack').addEventListener('click', () => {
    removeAttackCallback(newAttack);
  });
  tbody.appendChild(newAttack);
  return false;
}
function removeAttackCallback(attack) {
...
function pokemonMetaBoxes() {
  ...
  if (removeAttack) {
    for (let i = 0; i < removeAttack.length; i++) {
      removeAttack[i].addEventListener('click', () => {
        removeAttackCallback(removeAttack[i]);
      });
    }
  }
}
